...
module.exports = {
    ...
    parseFlags(flagsInSightInstance){
        let flagsInSight = {...flagsInSightInstance}
        let result = []
        for (const [key, value] of Object.entries(flagsInSight)) {
            let position = FlagsData[key]
            result.push({
                id: key,
    ...
    transferFlags(objectToTransfer, flagsInstance){
        ...
        flags.map((flag) => {
            let cos = Math.cos(toRadians(Math.abs(objectToTransfer.angle - flag.angle)))
            flag.distance = Math.sqrt(Math.abs(objectToTransfer.distance**2 + flag.distance**2 -
                                               2 * objectToTransfer.distance * flag.distance * cos))
        })
        return flags
    },
